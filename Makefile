...
help:
	...
	@echo ""
	@echo "■ Dépannage (make serve / LLMLingua, ex. No module named « llmlingua »)"
	@echo "  make doctor          → diagnostic (import Python, script worker, .gguf…)"
	@echo "  make doctor-fix      → désactive prompt_compression.enabled dans le YAML"
	@echo "  make llmlingua-venv  → crée un venv ; puis pip install -r scripts/requirements-llmlingua.txt"
	@echo "  Sinon YAML : prompt_compression.strict: false → serve démarre sans compression si le worker échoue"
	@echo ""
